<div>x
    <div class="flex justify-between gap-4">
        ...
        <div class="w-1/2">
            <x-mary-choices label="Zilla" wire:model.live="zilla_id" :options="$this->zillas" placeholder="Select Zilla" single
                searchable />

            <x-mary-choices label="Police Station" wire:model.live="thana_id" :options="$this->thanas"
                placeholder="Select Police Station" single searchable />

            <x-mary-choices label="Select Union" wire:model="union_id" :options="$this->unions" placeholder="Select Union"
                single searchable />

            <x-mary-datetime label="Start Date" wire:model="start_date" icon="o-calendar" />

            <x-mary-datetime label="End Date" wire:model="end_date" icon="o-calendar" />

            <x-mary-button wire:click="generate" label="Generate" class="btn-primary mt-1" />
        </div>
    ...
    <div>
        <x-mary-table :headers="$headers" with-pagination :rows="$this->crimes" striped>
...
@push('scripts')
    <script>
        window.addEventListener('refreshMap', function(event) {
            console.log('refreshMap', event.detail);
            //svg all path hidden
            ...
            let crimeTypeCount = event.detail[0].crimeTypeCount;
            let totalCrimeCount = event.detail[0].totalCrimeCount;

            let s = "";

            crimeTypeCount?.forEach(function(crimeType) {
                s += `${crimeType.crime_type} ${crimeType.count / totalCrimeCount * 100}%,`;
            });


            //svg path by id
            document.getElementById(event.detail[0].union?.toLowerCase())?.setAttribute('style', `background: linear-gradient(to right, red 50%,green 50%);display: inline;`);
            //document.getElementById(event.detail[0].thana?.toLowerCase())?.setAttribute('style', `background: linear-gradient(to right, ${s});display: inline;`);
            //document.getElementById(event.detail[0].zilla?.toLowerCase())?.setAttribute('style', `background: linear-gradient(to right, ${s});display: inline;`);
